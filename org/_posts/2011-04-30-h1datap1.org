#+TITLE:   Analysing the H-1B Data - Part 1
#+AUTHOR:    Ani
...
a good dataset to expore so I decided to download the H-1B iCert LCA
...
the table doesn't do the datatypes but it does give out the SQL /
create the table so that you can edit the table datatypes either in
...
glamourous as Ruby, Python etc. it is kind of awesome in its own way
...
expexted dip in the H-1B filings during the recession(2009). I'll be
